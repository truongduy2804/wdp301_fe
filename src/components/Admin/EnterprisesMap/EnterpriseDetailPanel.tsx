...
import { X, Building2, Phone, Mail, User, ShieldCheck, MapPin, Calendar, Trash2 } from "lucide-react";
...
const EnterpriseDetailPanel: React.FC<EnterpriseDetailPanelProps> = ({ enterpriseId, onClose }) => {
    const [enterprise, setEnterprise] = useState<EnterpriseDetailMap | null>(null);
                    ...
                    >
                        <div className="p-6">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-xl font-bold text-slate-900">Chi tiết doanh nghiệp</h2>
                                <button
                                    onClick={onClose}
                            ...
                            {isLoading ? (
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-5 flex items-start gap-4">
                                        ...
                                        <div>
                                            <h3 className="font-bold text-slate-900 text-lg leading-tight">{enterprise.name}</h3>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-slate-50 border border-slate-100 p-4 rounded-xl">
                                            <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mb-1">SĐT liên hệ</p>
                                            <p className="text-sm font-semibold text-slate-800 truncate">
                                                {enterprise.contactPhone || enterprise.phone || "---"}
                                            </p>
                                        </div>
                                        <div className="bg-slate-50 border border-slate-100 p-4 rounded-xl">
                                            <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mb-1">Công suất (Kg)</p>
                                            <p className="text-sm font-semibold text-slate-800">{enterprise.capacityKg || "---"}</p>
                                        </div>
                                        <div className="bg-slate-100/50 border border-slate-100 p-4 rounded-xl col-span-2">
                                            <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mb-1">Trạng thái hoạt động</p>
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex items-start gap-3 text-slate-700">
                                                ...
                                                <div className="space-y-1">
                                                    <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Email liên hệ</p>
                                                    <p className="text-sm font-medium leading-relaxed">{enterprise.contactEmail || enterprise.email || "---"}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-start gap-3 text-slate-700">
                                                ...
                                                <div className="space-y-1">
                                                    <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Địa chỉ</p>
                                                    <p className="text-sm font-medium leading-relaxed">{enterprise.address}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {/* Collectors */}
                                    {Array.isArray(enterprise.collectors) && enterprise.collectors.length > 0 && (
                                        <div className="space-y-4 pt-2">
                                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-[2px]">Nhân viên thu gom ({enterprise.collectors.length})</h4>
                                            <div className="grid grid-cols-1 gap-2">
                                                {enterprise.collectors.map((collector, idx) => (
                                                    <div key={idx} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                        <div className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden">
                                                            {collector.avatar ? <img src={collector.avatar} alt="" className="w-full h-full object-cover" /> : <User className="w-4 h-4 text-slate-400" />}
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-sm font-bold text-slate-900 truncate">{collector.fullName}</p>
                                                            <p className="text-[11px] text-slate-500 font-medium">{collector.phone}</p>
                                                        </div>
