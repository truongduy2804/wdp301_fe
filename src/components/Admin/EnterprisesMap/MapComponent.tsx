...
const MapController: React.FC<{ flyTo: { lat: number; lng: number } | null }> = ({ flyTo }) => {
    const map = useMap();
...
const MapComponent: React.FC<MapComponentProps> = ({ enterprises, isLoading, onSelectEnterprise, flyTo }) => {
    console.log("MapComponent Rendering with markers count:", (enterprises || []).length);
            ...
            >
                ...
                {(enterprises || []).map((ent) => (
                    <Marker
                        ...
                        eventHandlers={{
                            click: () => onSelectEnterprise(ent.id, ent.latitude, ent.longitude),
                        }}
                    >
